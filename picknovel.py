...
class PickNovel:
    def __init__(self, cookie_pn, cookie_hi):
        self.headers = {
            "Cookie": cookie_pn,
        ...
        self.headers_hinovel = {
            "Cookie": cookie_hi,
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36",
            "X-Requested-With": "XMLHttpRequest",
        }

    def hi_sync_pn(self, bids):
        # hi小说同步到picknovel
        ...
        response = requests.post(sync_api, headers=self.headers_hinovel, data=post_data).text
        return response

    def piliang_edit_book_status(self, pid):
        # 批量审核小说上架
    ...
    def piliang_edit_book_price(self, pid, kadian, price_type=1, price=43):
        # 批量小说改价
    ...
    def get_column_info(self, column_id, group_type=1):
        # 获取栏目全部小说（包括上架和下架）
        ...
        response = requests.get(api, params=params, headers=self.headers).text
    ...
    def edit_book(self, cid, pid, status=0, order=1, hot=0, rid=0, lang="en"):
        # 编辑栏目小说(新增或修改)
        # cid 栏目ID
        # pid PN的小说ID
        # status 0上架 1下架
