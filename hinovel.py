...
def get_user_info(uid):
    # 获取单个用户的用户信息列表
    api = f"https://manage.hinw2a.com/user/ajaxList?page=1&limit=10&is_vip=-1&id={uid}&os_type=0&create_time=&os_uuid=&account_type=-1&last_login_ip=&email=&is_email=-1&country=&is_pay_user=-1&last_login_time=&last_pay_time=&total_pay=&status=-1&risk_status=-1&deeplink=&tag_user_crowd=-1"
    headers = {"Cookie": cookie_hn}
    rsp = requests.get(url=api, headers=headers).json()
    df_user_info = pd.DataFrame(rsp["data"])
    return df_user_info
